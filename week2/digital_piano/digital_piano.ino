 ...
 * Author: [YOUR NAME]
 * Date: [DATE]
 * Description: 4 push buttons mapped to musical tones (C, D, E, F notes).
 *              Simultaneous 2-button press plays Sol (392Hz) bonus feature.
 *              Buzzer/piezo speaker silences on button release.
 *              Serial output displays active notes.
 */
...
void loop() {
  ...
  // Mode toggle comment (educational: Major vs Minor frequency adjustment)
  if (major_mode) {
    // Major mode uses standard frequencies (unchanged)
    // Comment: In Major mode, intervals follow the major scale pattern
  } else {
    // Minor mode would apply frequency shifts (example only)
    // Comment: In Minor mode, the 3rd and 6th scale degrees are lowered
    // Example adjustment (not applied to this implementation):
    // if (frequency == FREQ_E) frequency = 311; // E minor would be ~311Hz (E♭)
  }
