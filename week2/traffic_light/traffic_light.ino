 ...
 * Author: [YOUR NAME]
 * Date: [DATE]
 * Description: Implements a finite state machine (FSM) for traffic light control.
 *              Red (5s) → Yellow (2s) → Green (4s), with pedestrian button forcing
 *              Red for 8 seconds. Uses non-blocking millis() timing throughout.
 *              Bonus: +1 mark for millis()-based non-blocking timing (no delay() calls).
 */
