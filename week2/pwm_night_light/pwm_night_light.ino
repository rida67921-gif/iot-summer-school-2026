 ...
 * Author: [YOUR NAME]
 * Date: [DATE]
 * Description: LED breathing animation with 3 switchable modes:
 *              Mode 1: Slow breathing (3-second cycle)
 *              Mode 2: Fast pulse (0.5-second cycle)
 *              Mode 3: SOS pattern (...--- ...)
 *              Button cycles through modes. Uses millis() for non-blocking timing.
 */

#include <Arduino.h>

// Pin definitions
const int LED_PIN = 9;          // PWM-capable pin (digital PWM output)
...
// Breathing animation using PWM (sine-like approximation)
